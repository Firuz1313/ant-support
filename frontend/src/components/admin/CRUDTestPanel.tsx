...
const CRUDTestPanel: React.FC = () => {
    ...
    try {
      ...
      if (createResponse.ok) {
        ...
        const updateResponse = await fetch(
          `/api/v1/devices/${created.data.id}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  const testProblemOperations = async () => {
    ...
    try {
      ...
      if (createResponse.ok) {
        ...
        const createError = await createResponse.json();
        addTestResult(
          `❌ CREATE проблема failed: ${createError.error || createError.details}`,
        );
      }
    } catch (error) {
  ...
  const testEntityRelationships = async () => {
    ...
    try {
      ...
      if (devices.length > 0) {
        const deviceWithProblems = await fetch(
          `/api/v1/devices/${devices[0].id}?include_stats=true`,
        );
        if (deviceWithProblems.ok) {
          const data = await deviceWithProblems.json();
          addTestResult(
            `✅ Устройство ${devices[0].name}: найдено ${data.data?.problems_count || 0} проблем`,
          );
        }
      }
      ...
      if (devices.length > 0) {
        const problemsByDevice = await fetch(
          `/api/v1/problems?device_id=${devices[0].id}`,
        );
        if (problemsByDevice.ok) {
          const data = await problemsByDevice.json();
          addTestResult(
            `✅ Проблемы для ${devices[0].name}: ${data.data?.length || 0} найдено`,
          );
        }
      }
      ...
      if (problems.length > 0) {
        const stepsByProblem = await fetch(
          `/api/v1/steps?problem_id=${problems[0].id}`,
        );
        if (stepsByProblem.ok) {
          const data = await stepsByProblem.json();
          addTestResult(
            `✅ Шаги для проблемы ${problems[0].title}: ${data.data?.length || 0} найдено`,
          );
        }
      }
    } catch (error) {
          ...
          <div className="max-h-40 overflow-y-auto bg-gray-50 p-3 rounded text-sm">
            {testResults.length === 0 ? (
              <div className="text-gray-500">
                Нажмите кнопки выше для тестирования CRUD операций
              </div>
            ) : (
