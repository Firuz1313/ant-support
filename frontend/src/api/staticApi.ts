...
export class StaticApiService {
  ...
  private static diagnosticSteps = [
    ...
    },
    {
      ...
      instruction:
        "Проверьте аудио выходы приставки и входы телевизора/аудиосистемы",
      estimated_time: 60,
      is_active: true,
      created_at: "2025-08-18T10:00:00Z",
      updated_at: "2025-08-18T10:00:00Z",
    },
    {
      ...
      instruction:
        "Проверьте подключение блока питания и индикаторы на уст��ойстве",
      estimated_time: 30,
      is_active: true,
      created_at: "2025-08-18T10:00:00Z",
      updated_at: "2025-08-18T10:00:00Z",
    ...
    },
    {
      ...
      instruction:
        "Переключите телевизор на правильный вход (HDMI1, HDMI2, AV)",
      estimated_time: 30,
      is_active: true,
      created_at: "2025-08-18T10:00:00Z",
      updated_at: "2025-08-18T10:00:00Z",
    ...
    },
    {
      ...
      instruction:
        "Проверьте уровень громкости в меню приставки и на телевизоре",
      estimated_time: 90,
      is_active: true,
      created_at: "2025-08-18T10:00:00Z",
      updated_at: "2025-08-18T10:00:00Z",
  ...
  static async getProblemStats() {
    ...
    const published = this.problems.filter(
  ...
  static async getActiveSessions() {
    await this.simulateDelay();
    const activeSessions = this.diagnosticSessions.filter(
      (s) => s.is_active && !s.end_time,
    );
    return {
      success: true,
      data: activeSessions,
  ...
  static async getTVInterfacesByDevice(deviceId: string) {
    await this.simulateDelay();
    const deviceInterfaces = this.tvInterfaces.filter(
      (tv) => tv.device_id === deviceId,
    );
    return {
      success: true,
  ...
  static async getTVInterfaceStats() {
    ...
    const mainMenus = this.tvInterfaces.filter(
      (tv) => tv.type === "main_menu",
    ).length;
    const settings = this.tvInterfaces.filter(
      (tv) => tv.type === "settings",
    ).length;
